...
public class SymbolicExecution implements ISymbolicExecution {
	...
	public SymbolicExecution(ITestpath testpath, Parameter paramaters, IFunctionNode functionNode) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		IFullTestpath randomTestpath = tpGen.getPossibleTestpaths().get(2);
	...
	private void symbolicTestpath(ITestpath testpath, VariableNodeTable table, IFunctionNode function)
			throws Exception {
					...
					try {
						...
						if (cfgNode instanceof NormalCfgNode) {
							...
							case CONDITION: {
								...
								for (PathConstraint constraint : (PathConstraints) this.constraints)
									this.usedVariables
	...
	private Set<String> getUsedArrayvsPointerItem(String normalizedExpression) {
		...
		List<ICPPASTArraySubscriptExpression> arrayItems = Utils
				.getArraySubscriptExpression(Utils.convertToIAST(normalizedExpression));
		...
		for (ISymbolicVariable var : tableMapping)
			if (var instanceof BasicSymbolicVariable)
				...
				for (LogicCell logicCell : logicCells) {
					List<ICPPASTArraySubscriptExpression> arrayItems = Utils
							.getArraySubscriptExpression(Utils.convertToIAST(logicCell.getPhysicalCell().getValue()));

					String fullNameVar = var.getName() + logicCell.getFullIndex();
					arrayItems.addAll(Utils.getArraySubscriptExpression(Utils.convertToIAST(fullNameVar)));
	...
	private boolean parseSingleCondition(ICfgNode stm, IASTNode ast, int count, VariableNodeTable table)
		...
		if (ast instanceof ICPPASTLiteralExpression || ast instanceof IASTIdExpression) {
			...
			if (right.equals("NULL")) {
				...
				ConditionParser conditionParser = new ConditionParser();
				conditionParser.parse(Utils.convertToIAST(exp), table);
				/*
	...
	@Override
	public void setTestpath(ITestpath testpath) {
		this.testpath = testpath;
	}
}
